 ...
 */
public final class TFVectorsUseFixedDictionary extends AbstractJob {
  
  private static final Logger log = LoggerFactory.getLogger(TFVectorsUseFixedDictionary.class);
  
  public static void main(String[] args) throws Exception {
	  String[] newArgs = {"-i", args[0], "-o", args[1], "-ow", 
				"-chunk", "128", "-wt","tf", "-s", "3", "-md",
				"2", "-x", "70", "-ng", "1", "-ml", "50", "-seq", "-n", "2"};
    ToolRunner.run(new TFVectorsUseFixedDictionary(), newArgs);
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    try {
      ...
      String tfDirName = shouldPrune ? FixDictionaryVectorizer.DOCUMENT_VECTOR_OUTPUT_FOLDER+"-toprune" : FixDictionaryVectorizer.DOCUMENT_VECTOR_OUTPUT_FOLDER;

      if (!processIdf) {
        FixDictionaryVectorizer.createTermFrequencyVectors(tokenizedPath, outputDir, tfDirName, conf, minSupport, maxNGramSize,
      ...
      Pair<Long[], List<Path>> docFrequenciesFeatures = null;
       ...
       if (shouldPrune) {
         ...
         Path prunedPartialTFDir = new Path(outputDir, FixDictionaryVectorizer.DOCUMENT_VECTOR_OUTPUT_FOLDER
                 + "-partial");
         if (processIdf) {
      ...
      if (processIdf) {
          TFIDFConverter.processTfIdf(
                 new Path(outputDir, FixDictionaryVectorizer.DOCUMENT_VECTOR_OUTPUT_FOLDER),
