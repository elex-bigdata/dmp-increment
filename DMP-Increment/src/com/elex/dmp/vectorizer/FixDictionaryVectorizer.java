 ...
 */
public final class FixDictionaryVectorizer implements Vectorizer{
  ...
  public static void createTermFrequencyVectors(Path input,
    ...
    //使用上次离线计算形成的词典进行词频统计，指定了备份目录下的字典目录作为输入路径
    ...
    if (maxNGramSize == 1) {
      ...
      dictionaryChunks = createDictionaryChunks(dictFilePath, output, baseConf, chunkSizeInMegabytes, maxTermDimension);
    } else {
      CollocDriver.generateAllGrams(input, dictionaryJobPath, baseConf, maxNGramSize,
        minSupport, minLLRValue, numReducers);
      //两次向量转换过程中需要使用同样的ngram参数
      dictionaryChunks = createDictionaryChunks(dictFilePath,output,baseConf,chunkSizeInMegabytes,maxTermDimension);
    }
   ...
   */
  private static List<Path> createDictionaryChunks(Path dictPath,
