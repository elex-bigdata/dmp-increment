...
public class TopicModel implements Configurable, Iterable<MatrixSlice> {
  ...
  public static Pair<Matrix, Vector> loadModel(Configuration conf, Path... modelPaths)
    ...
    for (Path modelPath : modelPaths) {
      for (Pair<Text, VectorWritable> row :
          new SequenceFileIterable<Text, VectorWritable>(modelPath, true, conf)) {
        rows.add(Pair.of(Integer.parseInt(row.getFirst().toString()), row.getSecond().get()));//����ģ���ļ��е�keyΪtext�ͣ������Ҫ��ת��
        numTopics = Math.max(numTopics, Integer.parseInt(row.getFirst().toString()));//����ģ���ļ��е�keyΪtext�ͣ������Ҫ��ת��
        if (numTerms < 0) {
